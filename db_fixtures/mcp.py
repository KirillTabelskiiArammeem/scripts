...
"mail_message_res_partner_starred_rel", "mail_resend_message", "message_attachment_rel"]

MCP_TABLES = ["helpdesk_ticket", "aram_partner_product_vo", "aram_partner_pos_vo"]
...
def get_connection():
    conn = connect(
        host="db", port=5432, user="odoo", password="odoo", database="mcp"
    )
...
def clean_mcp_tables():
    return delete_from_tables(MCP_TABLES)


def thin_out_ir_model_data():
    models = ", ".join(f"'{item.replace('_', '.')}'" for item in TABLES_TO_DROP + WEB_TABLES_TO_DROP + MAIL_TABLES_TO_DROP + MCP_TABLES)
    query = f"""
...
def backup(path="fixture.sql"):
    ...
    check_call(
        f"pg_dump --file '{path}' --host '127.0.0.1' --port '5432' --username 'odoo' --format=p 'mcp'",
        shell=True,
    )
    with ZipFile(
